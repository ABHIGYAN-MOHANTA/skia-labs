...
import { collection, getDocs, doc, updateDoc, increment, setDoc, query, where, arrayUnion, arrayRemove, deleteDoc } from 'firebase/firestore';
...
function ShaderPreview({ code }: { code: string }) {
    ...
    useEffect(() => {
        ...
        const init = async () => {
            ...
            const drawFrame = () => {
                if (!surface || !effect || !canvasRef.current) return;
                ...
                if (shader && canvasRef.current) {
                    paint.setShader(shader);
                    canvas.drawRect(ck.LTRBRect(0, 0, canvasRef.current.width, canvasRef.current.height), paint);
                    surface.flush();
                    shader.delete();
                }
                paint.delete();
...
export default function CommunityPage() {
    ...
    useEffect(() => {
        const fetchShaders = async () => {
            try {
                const querySnapshot = await getDocs(collection(db, 'shaders'));
                ...
                shadersData.sort((a, b) => b.likes - a.likes);
